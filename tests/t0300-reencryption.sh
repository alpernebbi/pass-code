...
test_expect_failure 'Root key encryption' '
...
test_expect_failure 'Reencryption root single key' '
	"$PASS" init $KEY2 &&
	[[ $(canonicalize_gpg_keys "$KEY2") == "$(gpg_keys_from_encrypted_file "$PASSWORD_STORE_DIR/folder/cred1.gpg")" ]]
'

test_expect_failure 'Reencryption root multiple key' '
...
test_expect_failure 'Reencryption root multiple key with string' '
...
test_expect_failure 'Reencryption root group and other keys' '
...
test_expect_failure 'Reencryption root group to identical individual with no file change' '
